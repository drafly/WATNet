...
class MaxViTTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        self.mlp = DualFeed(in_channels, mlp_ratio, bias=True)
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        """
        # Save original shape
        B, C, H, W = input.shape # [2,16,400,600]
        # Perform partition
        input_partitioned = self.partition_function(input, self.grid_window_size) #[7500,8,8,16]
        input_partitioned = input_partitioned.view(-1, self.grid_window_size[0] * self.grid_window_size[1], C) #[15000,64,3]
        # Perform normalization, attention, and dropout
        output = input_partitioned + self.drop_path(self.attention(self.norm_1(input_partitioned))) #[7500,64,16]
        # Perform normalization, MLP, and dropout
        # output = output + self.drop_path(self.mlp2(self.norm_2(output)))
        # # Reverse partition
        # output = self.reverse_function(output, (H, W), self.grid_window_size)
        output = self.reverse_function(output, (H, W), self.grid_window_size)

        output = output + self.drop_path(self.mlp(output))
        # # Reverse partition
